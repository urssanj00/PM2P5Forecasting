# -*- coding: utf-8 -*-
"""sensor_id_updated-01-FEB-2025.ipynb
...
# Example inference (replace with actual data)
with torch.no_grad():
    forecast_predictions = model(X_forecast_tensor, edge_index, X_forecast_tensor)

# Initialize predicted_pm2p5 column with NaN
df['predicted_pm2p5'] = np.nan

# Calculate metrics for training set
predictions_train_unscaled = np.concatenate([scalers_y[sensor_id].inverse_transform(best_train_predictions[idx:idx+len(df[df['sensor_id'] == sensor_id])-n_steps+1].detach().numpy().reshape(-1, 1)).flatten() for idx, sensor_id in enumerate(sensor_ids)], axis=0)
y_train_unscaled = np.concatenate([scalers_y[sensor_id].inverse_transform(y_train[idx:idx+len(df[df['sensor_id'] == sensor_id])-n_steps+1].reshape(-1, 1)).flatten() for idx, sensor_id in enumerate(sensor_ids)], axis=0)

print(f'{y_train_unscaled} \t {predictions_train_unscaled}')
# Calculate metrics for testing set
predictions_test_unscaled = np.concatenate([scalers_y[sensor_id].inverse_transform(best_test_predictions[idx:idx+len(df[df['sensor_id'] == sensor_id])-n_steps+1].detach().numpy().reshape(-1, 1)).flatten() for idx, sensor_id in enumerate(sensor_ids)], axis=0)
y_test_unscaled = np.concatenate([scalers_y[sensor_id].inverse_transform(y_test[idx:idx+len(df[df['sensor_id'] == sensor_id])-n_steps+1].reshape(-1, 1)).flatten() for idx, sensor_id in enumerate(sensor_ids)], axis=0)
print(f'{y_test_unscaled} \t {predictions_test_unscaled}')
df['predicted_pm2p5'] = predictions_train_unscaled.append( predictions_test_unscaled)
'''
# Unscale best predictions and append to DataFrame based on sensor_id
index = 0
print (f'sensor_ids:{sensor_ids}')
for sensor_id in sensor_ids:

    df_sensor = df[df['sensor_id'] == sensor_id].copy()
    print (f'df_sensor:{df_sensor}')

    scaler_y = scalers_y[sensor_id]

    num_predictions = len(best_test_predictions[index:index+len(df_sensor)-n_steps+1])
    predictions_unscaled = scaler_y.inverse_transform(best_test_predictions[index:index+num_predictions].detach().numpy().reshape(-1, 1)).flatten()

    for i in range(num_predictions):
        df.loc[df_sensor.index[n_steps + i - 1], 'predicted_pm2p5'] = predictions_unscaled[i]

    index += num_predictions

print('Actual and Predicted PM2p5')
print(df)
'''
# Save the DataFrame to a CSV file
op_path = '/content/drive/MyDrive/Intellipaat-sem2/AirPollutionAnalysis/RG/pm2p5_output'
filename = 'predicted_data_with_sensor_id.csv'
df.to_csv(f'{op_path}/{filename}', index=False)

print(f"DataFrame has been successfully saved to {op_path}/{filename}")
